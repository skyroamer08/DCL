...
def main():
    ...
    model_clip, preprocess = clip.load(args.vision_model, device=device, jit=False) #Must set jit=False for training
    ...
    for epoch in range(args.epochs):
        ...
        for idx, batch in enumerate(train_dataloader) :
            ...
            img_norm = img_embs.norm(dim=-1, keepdim=True )
            text_embs = model.encode_text(texts)
            text_norm = text_embs.norm(dim=-1, keepdim=True )

            scores = ((img_embs/img_norm) @ (text_embs/text_norm).T).type(torch.float64)
            loss = CCL(args.tau, args.loss_cmcl, args.loss_gce, args.threshold, args.margin, args.balance1, args.balance2)
            total_loss = loss(scores, args.bmm)
